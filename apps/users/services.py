...
from common.constants import MAX_OTP_ATTEMPTS, OTP_EXPIRATION_MINUTES
...
def generate_otp(email, purpose="signup"):
    ...
    otp_obj = OTP.objects.create(email=email, otp_code=otp_code, purpose=purpose, expires_at=expires_at)
    return otp_obj
...
def verify_signup_otp(email, otp_code):
    ...
    latest_otp = OTP.objects.filter(email=email, purpose="signup", is_used=False).order_by("-created_at").first()
