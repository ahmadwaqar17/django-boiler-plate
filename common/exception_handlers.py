...
def custom_exception_handler(exc, context):
    ...
    logger.error(f"Exception in view {view.__class__.__name__} at {path}: {exc!s}", exc_info=True)

    # We build the custom payload
    formatted_response = {
        "success": False,
        "error": {"message": "A server error occurred.", "code": status.HTTP_500_INTERNAL_SERVER_ERROR},
        "data": {},
    }
    ...
    if settings.DEBUG:
        formatted_response["error"]["message"] += f" Detail: {exc!s}"
